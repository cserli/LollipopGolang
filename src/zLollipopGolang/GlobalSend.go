...
//发送给客户端的数据的函数
func (this *OnlineServer) PlayerSendServerMessage(senddata interface{}) int {
...
// 游戏列表的关系
func GetGameIdByGameName(strGameName string) int {

	if G_GameInfoST[strGameName] != nil {
		return int(G_GameInfoST[strGameName].GameId)
	}
	//	iret := dbif.G_mapGameListInfo[strGameName].GameId
	//	if iret != 0 {
	//		return int(iret)
	//	}
	glog.Error("Entry GetGameIdByGameName is Fail!!!")
	glog.Flush()
